...
class Focal_MultiLabel_Loss(nn.Module):
    ...
    def forward(self, outputs, targets):
        bce = self.bceloss(nn.Sigmoid()(outputs), targets)
...
class GeneralizedRCNN(nn.Module):
    ...
    def forward(self, images, targets=None, labels_one_hot=None):
        # type: (List[Tensor], Optional[List[Dict[str, Tensor]]], Tensor) -> Tuple[Dict[str, Tensor], List[Dict[str, Tensor]], Tensor]
        """
        Args:
        ...
        features, pred = self.backbone(images.tensors)
        ...
        if labels_one_hot is not None:
            bce_losses = self.criterion(pred, labels_one_hot)

        losses = {}
        losses.update(detector_losses)
        losses.update(proposal_losses)
        if labels_one_hot is not None:
            losses.update({"bce_losses": bce_losses})
