...
import detection.mobilenet, detection.resnet
from detection._api import _get_enum_from_fn, WeightsEnum
from detection._utils import handle_legacy_interface, IntermediateLayerGetter
...
### 改良版 ###
#############
class BackboneWithFPN_V2(nn.Module):
    def __init__(
        self,
        backbone: nn.Module,
        return_layers: Dict[str, str],
    ...
    def forward(self, x: Tensor) -> Dict[str, Tensor]:
        body_feature = self.body(x)
        x_fpn = self.fpn(body_feature)
        x_fc = self.AdaptiveAvgPool2d(body_feature["3"])
...
def resnet_fpn_backbone(
    *,
    ...
    """
    backbone = detection.resnet.__dict__[backbone_name](weights=weights, norm_layer=norm_layer)
    return _resnet_fpn_extractor(backbone, trainable_layers, returned_layers, extra_blocks)


# def _resnet_fpn_extractor(
#     backbone: detection.resnet.ResNet,
...
def _resnet_fpn_extractor(
    ...
    # )
    return BackboneWithFPN_V2(
        backbone, return_layers, in_channels_list, out_channels, extra_blocks=extra_blocks, norm_layer=norm_layer, num_classes=num_classes
    )
...
def mobilenet_backbone(
    *,
    ...
    backbone = detection.mobilenet.__dict__[backbone_name](weights=weights, norm_layer=norm_layer)
    return _mobilenet_extractor(backbone, fpn, trainable_layers, returned_layers, extra_blocks)


def _mobilenet_extractor(
    backbone: Union[detection.mobilenet.MobileNetV2, detection.mobilenet.MobileNetV3],
    fpn: bool,
    trainable_layers: int,
    returned_layers: Optional[List[int]] = None,
    extra_blocks: Optional[ExtraFPNBlock] = None,
